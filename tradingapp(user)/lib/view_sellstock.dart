...
void main() {
  runApp(const ViewSellstock());
}

class ViewSellstock extends StatelessWidget {
  const ViewSellstock({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return const MaterialApp(
      debugShowCheckedModeBanner: false,
      home: VStockSellListPage(title: 'Sold Stocks'),
    );
  }
}

class VStockSellListPage extends StatefulWidget {
  const VStockSellListPage({Key? key, required String title}) : super(key: key);

  @override
  State<VStockSellListPage> createState() => _VStockSellListPageState();
}

class _VStockSellListPageState extends State<VStockSellListPage> {
  ...
  Future<void> fetchSoldStocks() async {
    try {
      ...
      String sid = sh.getString('fid')?.toString() ?? '';

      print('User ID: $lid');
      print('Stock ID: $sid');

      String apiUrl = '$baseUrl/view_sell_fastock/';
